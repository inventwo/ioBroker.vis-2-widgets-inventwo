...
export default class InventwoWidgetSlider extends InventwoGeneric {
    constructor(props) {
        super(props);
        this.state = {
            ...this.state,
    ...
    renderWidgetBody(props) {
        ...
        return (_jsx(Slider, { disabled: this.props.editMode, sx: sliderAttributes, onChange: (_e, val) => this.setState({ sliderValue: val }), onChangeCommitted: (e, val) => this.setState({ sliderValue: val }, () => this.onChange(e, val)), min: this.state.rxData.minValue, max: this.state.rxData.maxValue, step: this.state.rxData.step, value: this.state.sliderValue || 0, valueLabelDisplay: "auto", track: trackStyle.trackBarType, orientation: this.state.rxData.orientation, marks: marks }));
    }
}
